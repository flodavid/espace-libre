...
public class EspaceLibre.VolumesManager : Object {
    ...
    public async void unmount_current () {
        ...
        if (current_volume != null && current_volume.mounted && current_volume.volume != null
            && current_volume.volume.get_mount () != null
        ) {
                unmount_mount.begin (current_volume.volume.get_mount (), (obj, res) => {
    ...
    public async bool mount_current () {
        ...
        // mount_mountable.begin (current_volume.activation_root, (obj, res) => {
        //     mount_mountable.end (res);
        //     refresh ();
        //     current_volume.mounted = res;
        //     if (res = false) current_volume.mount = null;
            // Hacky, to refresh currently selected
        //     current_volume = current_volume;
        // });
    ...
    private async bool mount_volume (VolumeEntry volume) {
        ...
        try {
            ...
            if (volume.volume != null) {
                if (volume.volume.get_activation_root () != null) {
                    ...
                    res = yield volume.volume.mount (GLib.MountMountFlags.NONE, mount_op, cancellable);
                }
            } else {
            ...
            warning ("first failure when trying to mount. %s", e.message);
                    ...
                    volume_elem = volume_elem.next)
            {
                ...
                if (!already_known) {
                    ...
                    try {
                        ...
                        var mount_path = mount_root != null ? mount_root.get_path () : null;
                        var volume_entry = new VolumeEntry (unix_device, mount_path, fs_type, "", "", "");
